...
def run_candidate(candidate: dict, bars_1m: list) -> dict:
...
def run_candidate_with_trades(candidate: dict, bars_1m: list) -> tuple:
...
def generate_report(results: list, all_trades: list):
    ...
    report_path = RESULTS_DIR / "YTD_BOX_STRATEGY_REPORT.md"
...
def main():
    ...
    if args.report_only:
        ...
        generate_report(results, all_trade_rows)
        return
    ...
    try:
        for di, date_str in enumerate(dates_sorted):
            ...
            for ci, candidate in enumerate(date_candidates):
                ...
                # Run strategy
                result, trades = run_candidate_with_trades(candidate, bars_1m)
