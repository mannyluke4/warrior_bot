...
def _assert_broker_matches() -> None:
    ...
    Unset WB_EXPECTED_BROKER = silently allow (no regression).
    """
...
def reconcile_positions_on_startup():
    ...
    for pos in positions:
        ...
        # not orphans. (Patched 2026-05-05 after squeeze orphan-adopt stole every
        # WB entry today and force-closed via bail_timer.)
        if symbol in state.wb_positions:
...
def periodic_position_sync():
    ...
    if not state.open_position:
        for symbol, pos in broker_symbols.items():
            ...
            # (Patched 2026-05-05 after squeeze stole every WB entry today.)
            if symbol in state.wb_positions:
                continue
...
def seed_symbol_from_cache(symbol: str) -> bool:
    ...
    cache_path = os.path.join(ss.tick_cache_dir(today), f"{symbol}.json.gz")
...
def run_scanner():
    ...
    Kept as a named no-op so existing call sites in the main loop work
    without restructuring."""
    return


# Path to the MAIN bot's watchlist file. Hardcoded to "session_state" (NOT
# WB_SESSION_DIR_NAME) — the sub-bot reads what the main bot wrote, not its
# own isolated state. Resolved per-call so the date rolls over correctly
# at midnight.
def _main_bot_watchlist_path() -> str:
    today = datetime.now(ET).strftime("%Y-%m-%d")
    return os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "session_state", today, "watchlist.json",
    )


def poll_watchlist():
    """Mirror the main bot's subscriptions onto the Alpaca feed.

    Reads session_state/<today>/watchlist.json — the file the main bot
    writes (via persist_watchlist) after every successful subscribe_symbol.
    For each symbol the main bot watches that this sub-bot does not, run
    the same subscribe_symbol() flow (qualify → reqMktData → init detectors
    → seed). This keeps the sub-bot's universe and seeding-window aligned
    with the main bot's, so any divergence in trades is attributable to
    the data feed, not to symbol selection.

    Symbols the main bot drops are NOT unsubscribed here — same invariant
    as the main bot's poll_watchlist (once subscribed, stays subscribed
    for the session)."""
    path = _main_bot_watchlist_path()
    ...
    try:
        ...
        print(f"⚠️ Sub-bot: read main bot watchlist failed: {e}", flush=True)
        return
    ...
    if new_syms:
        print(f"\n📡 Sub-bot: {len(new_syms)} new symbols from main bot's watchlist: "
              f"{sorted(new_syms)}", flush=True)
        for sym in new_syms:
            try:
                subscribe_symbol(sym)
            except Exception as e:
                print(f"⚠️ Sub-bot: subscribe {sym} failed: {e}", flush=True)
                traceback.print_exc()
            # Heartbeat between subscribes — historical fetch + WS subscribe
            # can each take a few seconds; many sequential subs would blow
            # past the 120s watchdog.
            update_heartbeat()
...
def save_tick_cache(source: dict | None = None):
    ...
    today = datetime.now(ET).strftime("%Y-%m-%d")
    cache_dir = ss.tick_cache_dir(today)
    ...
    if saved:
        cache_name = os.getenv("WB_TICK_CACHE_DIR_NAME", "tick_cache")
        print(f"📦 Tick cache saved: {saved} symbols → {cache_name}/{today}/", flush=True)
